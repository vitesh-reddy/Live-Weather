...
let changeTab = (Event) => {
    ...
    if (element == "search" || element == "searchbar" || element == "searchbtn") {
        searchbar.focus();
        ...
        if (yourCityName == undefined) {
            ...
            setTimeout(() => {
                permission.querySelector("button").classList.remove("scale-105");
            }, 200);
            return;
        }
        getYourLocation();
...
let setYourCityName = async (latitude, longitude) => {
    let responce = await fetch("https://geocode.maps.co/reverse?lat=" + latitude + "&lon=" + longitude + "&api_key=668d312d38652220816457cjnb80033");
    let data = await responce.json();

    cityName = (data.address?.suburb != undefined) ? data.address?.suburb : data.address?.county;
...
let getSearchCity = async () => {
    ...
    const response = await fetch('https://geocode.maps.co/search?q=' + cityName + '&api_key=668d312d38652220816457cjnb80033');
...
let getCityData = async (latitude, longitude, native) => {
    let coordsResponce = await fetch('https://api.openweathermap.org/data/2.5/weather?lat=' + latitude + "&lon=" + longitude + '&appid=' + API_KEY + '&units=metric');
...
let setCity = async () => {
    ...
    if (await cityName.charCodeAt(0) >= 95)
        city.querySelector("#cityname").innerText = String.fromCharCode(await cityName.charCodeAt(0) - 32) + cityName.slice(1);
    else
        city.querySelector("#cityname").innerText = cityName;

    let flag = cityData.sys.country;
    let flagUrl = "https://flagsapi.com/" + flag + "/flat/32.png"
    let flagurl = "https://www.worldometers.info//img/flags/small/tn_" + flag + "-flag.gif"
    city.querySelector("#flag").setAttribute("src", flagUrl);

    let sky = cityData.weather[0].main;
    city.querySelector("#sky").innerText = sky;

    let skyimgurl = "http://openweathermap.org/img/w/" + cityData.weather[0].icon + ".png"
    ...
    city.querySelector("#windspeed").innerText = (cityData.wind.speed * 3.6).toFixed(2) + " Kmph";
    ...
    else if (5000 <= visibleRange < 10000)
        visibility = "Good";
    else if (2000 <= visibleRange < 5000)
        visibility = "Fair";
    else if (1000 <= visibleRange < 2000)
        visibility = "Poor";
    else
